"""LoaderV301: loader_v301 module v2.
...
@dataclass
class LoaderV301Config:
    """Configuration for loader_v301 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV301(nn.Module):
    """Neural network module for loader_v301 v2.
    ...
    def __init__(self, config: Optional[LoaderV301Config] = None):
        super().__init__()
        self.config = config or LoaderV301Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV301 v2 with %d params",
