"""LoaderV331: loader_v331 module v2.
...
@dataclass
class LoaderV331Config:
    """Configuration for loader_v331 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV331(nn.Module):
    """Neural network module for loader_v331 v2.
    ...
    def __init__(self, config: Optional[LoaderV331Config] = None):
        super().__init__()
        self.config = config or LoaderV331Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV331 v2 with %d params",
