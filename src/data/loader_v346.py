"""LoaderV346: loader_v346 module v2.
...
@dataclass
class LoaderV346Config:
    """Configuration for loader_v346 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV346(nn.Module):
    """Neural network module for loader_v346 v2.
    ...
    def __init__(self, config: Optional[LoaderV346Config] = None):
        super().__init__()
        self.config = config or LoaderV346Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV346 v2 with %d params",
