"""RoutesV320: routes_v320 module v1.
...
@dataclass
class RoutesV320Config:
    """Configuration for routes_v320 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class RoutesV320(nn.Module):
    """Neural network module for routes_v320 v1.
    ...
    def __init__(self, config: Optional[RoutesV320Config] = None):
        super().__init__()
        self.config = config or RoutesV320Config()
        c = self.config
        ...
        logger.info("Initialized RoutesV320 v1 with %d params",
