"""RoutesV305: routes_v305 module v1.
...
@dataclass
class RoutesV305Config:
    """Configuration for routes_v305 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class RoutesV305(nn.Module):
    """Neural network module for routes_v305 v1.
    ...
    def __init__(self, config: Optional[RoutesV305Config] = None):
        super().__init__()
        self.config = config or RoutesV305Config()
        c = self.config
        ...
        logger.info("Initialized RoutesV305 v1 with %d params",
