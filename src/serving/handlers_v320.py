"""HandlersV320: handlers_v320 module v2.
...
@dataclass
class HandlersV320Config:
    """Configuration for handlers_v320 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class HandlersV320(nn.Module):
    """Neural network module for handlers_v320 v2.
    ...
    def __init__(self, config: Optional[HandlersV320Config] = None):
        super().__init__()
        self.config = config or HandlersV320Config()
        c = self.config
        ...
        logger.info("Initialized HandlersV320 v2 with %d params",
