"""AlertingTransformsV2S22: alerting_transforms_v2_s22 module v2.
...
@dataclass
class AlertingTransformsV2S22Config:
    """Configuration for alerting_transforms_v2_s22 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class AlertingTransformsV2S22(nn.Module):
    """Neural network module for alerting_transforms_v2_s22 v2.
    ...
    def __init__(self, config: Optional[AlertingTransformsV2S22Config] = None):
        super().__init__()
        self.config = config or AlertingTransformsV2S22Config()
        c = self.config
        ...
        logger.info("Initialized AlertingTransformsV2S22 v2 with %d params",
