"""ConfigV323: config_v323 module v1.
...
@dataclass
class ConfigV323Config:
    """Configuration for config_v323 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ConfigV323(nn.Module):
    """Neural network module for config_v323 v1.
    ...
    def __init__(self, config: Optional[ConfigV323Config] = None):
        super().__init__()
        self.config = config or ConfigV323Config()
        c = self.config
        ...
        logger.info("Initialized ConfigV323 v1 with %d params",
