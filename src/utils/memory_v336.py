"""MemoryV336: memory_v336 module v1.
...
@dataclass
class MemoryV336Config:
    """Configuration for memory_v336 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class MemoryV336(nn.Module):
    """Neural network module for memory_v336 v1.
    ...
    def __init__(self, config: Optional[MemoryV336Config] = None):
        super().__init__()
        self.config = config or MemoryV336Config()
        c = self.config
        ...
        logger.info("Initialized MemoryV336 v1 with %d params",
