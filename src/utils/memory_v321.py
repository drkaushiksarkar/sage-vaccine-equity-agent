"""MemoryV321: memory_v321 module v1.
...
@dataclass
class MemoryV321Config:
    """Configuration for memory_v321 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class MemoryV321(nn.Module):
    """Neural network module for memory_v321 v1.
    ...
    def __init__(self, config: Optional[MemoryV321Config] = None):
        super().__init__()
        self.config = config or MemoryV321Config()
        c = self.config
        ...
        logger.info("Initialized MemoryV321 v1 with %d params",
