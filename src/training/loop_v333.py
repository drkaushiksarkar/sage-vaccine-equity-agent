...
@dataclass
class LoopV333Config:
    """Configuration for loop_v333 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class LoopV333(nn.Module):
    """Neural network module for loop_v333 v1.
    ...
    def __init__(self, config: Optional[LoopV333Config] = None):
        super().__init__()
        self.config = config or LoopV333Config()
        c = self.config
        ...
        logger.info("Initialized LoopV333 v1 with %d params",
