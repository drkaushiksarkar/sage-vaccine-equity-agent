"""GradientV321: gradient_v321 module v2.
...
@dataclass
class GradientV321Config:
    """Configuration for gradient_v321 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class GradientV321(nn.Module):
    """Neural network module for gradient_v321 v2.
    ...
    def __init__(self, config: Optional[GradientV321Config] = None):
        super().__init__()
        self.config = config or GradientV321Config()
        c = self.config
        ...
        logger.info("Initialized GradientV321 v2 with %d params",
